# -*- coding: utf-8 -*-
import scrapy
from ..items import LotteryqlcItem


class LotterySpider(scrapy.Spider):
    name = 'lottery'
    allowed_domains = ['zhcw.com']
    start_urls = ['http://kaijiang.zhcw.com/zhcw/html/qlc/list_1.html']
    index = 1
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item["red7"] = node.xpath("./td[3]/em[7]/text()").extract_first()
            item["blue"] = node.xpath("./td[3]/em[8]/text()").extract_first()

            yield item

        self.index += 1
        next_url = "http://kaijiang.zhcw.com/zhcw/html/qlc/list_{}.html".format(self.index)
